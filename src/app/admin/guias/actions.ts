...
function parseForm(formData: FormData) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      sections = parsed
        .filter((section) => section && String(section.heading).trim() && String(section.content).trim())
        .map((section) => ({
...
function validateGuide(data: ReturnType<typeof parseForm>) {
  if (!data.title || !data.slug || !data.category || !data.excerpt) {
    throw new Error("Completa título, slug, categoría y descripción.");
  }
...
export async function createGuide(formData: FormData) {
  ...
  if (data.sections.length) {
    const { error: sectionsError } = await supabase
      .from("guide_sections")
      .insert(data.sections.map((section, index) => ({
        guide_id: guide.id,
